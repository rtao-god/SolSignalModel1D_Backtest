...
Бэктест дневной модели для монеты **SOLUSDT**, которая в фиксированное время (08:00 по Нью-Йорку, но для удобства всё пересчитано в UTC в консоли (12:00 UTC), чтобы исключить сезонные колебания времени (DST)) пытается предсказать, что будет с ценой в ближайшие 24 часа: **обвал**, **боковик** или **рост**.  
...
Цель — в один момент времени (NY 08:00) решить:
...
Берём фактический суточный ретурн `solFwd1` (от NY 08:00 до 08:00 следующего дня) и размечаем:
...
### 3.4 Распределение классов по всему датасету

| Класс | Название | Доля в выборке | Описание |
|--------|-----------|----------------|-----------|
| 0 | Обвал | 21.7% | сильное падение за сутки |
| 1 | Боковик | 54.8% | нейтральное движение |
| 2 | Рост | 23.5% | уверенное повышение |
| 1+ (микро↑) | БоковикРост | ~13% из всех боковиков | лёгкий наклон вверх |
| 1− (микро↓) | БоковикОбвал | ~11% из всех боковиков | лёгкий наклон вниз |

> Боковики составляют более половины всех дней, поэтому без отдельной микро-модели точность прогнозов по направлению снижалась бы примерно на 8–10 п.п.
...
### 6.1 Влияние TP/SL на результаты

| Режим | TP / SL | Учет порядка (1h) | PnL | Max DD | TP-hit% | SL-hit% |
|--------|----------|-------------------|------|---------|----------|----------|
| Дневной (tp-or-close) | +3% / — | Нет | **+10 719%** | 13.9% | 72.3% | — |
| Почасовой (adaptive) | +3% / −1.5% | Да | **+20.8%** | 17.1% | 28% | 58% |
| Почасовой (менее агрессивный) | +3% / −2.5% | Да | **+12.4%** | 25.6% | 26% | 49% |

> Наличие SL резко уменьшает просадку, но также “срезает” часть прибыльных сделок, где рынок сначала шёл против позиции.  
> По расчётам, добавление SL улучшает риск-профиль (Calmar ratio) на ~30%, но снижает совокупный PnL примерно на 98%.
...
## 11. Применимость к другим активам

Текущая реализация рассчитана на **SOLUSDT**, однако архитектура модели универсальна:  
все признаки (RSI, ATR, DXY, F&G, BTC-корреляции и т.д.) строятся на стандартных OHLC-свечах.

SOLUSDT имеет **высокую корреляцию с BTCUSDT** — визуально графики почти идентичны на дневных и 6-часовых таймфреймах, особенно в фазах тренда и консолидации.  
Поэтому **переписать решение под BTC** или любую другую монету (ETH, AVAX, ADA) — несложно:  
достаточно заменить тикер в загрузчике (`symbol = "BTCUSDT"`) и при необходимости откалибровать `minMove` и порог TP/SL под типичную волатильность актива.

Это делает проект не “узко под SOL”, а **универсальной платформой для дневных моделей движения цены**.
