		...
		public static void LogDailyBarePnlWithBaselinesAndShuffle (
			...
			if (split.Excluded.Count > 0)
				...
				var sample = split.Excluded
					...
					.Select (r => r.Causal.DateUtc.ToCausalDateUtc().ToString ("O"));
		...
		private static List<double> CollectPnlSeries (
			...
			foreach (var r in records)
				...
				if (!HasValidPrices (r))
					{
					Console.WriteLine (
						$"[pnl-bare] WARNING: invalid prices for {r.Causal.DateUtc.ToCausalDateUtc():O}, Entry={r.Forward.Entry}, Close24={r.Forward.Close24}, skip day.");
					continue;
					}
