...
namespace SolSignalModel1D_Backtest.Core.Analytics.Backtest.Snapshots.ModelStats
	...
	public static class BacktestModelStatsSnapshotBuilder
		...
		private static void ComputeSlStats (
			...
			// Для скорости: один раз сортируем 1m и дальше режем диапазоны бинарным поиском.
			...
			foreach (var r in records)
				...
				// Если у записи нет решения SL (nullable) — такую запись нельзя корректно учитывать в confusion.
		...
		private static DayOutcome GetDayOutcomeFromMinutes (
			...
			// Weekend-entry: baseline-exit не определён → день исключаем из path-истины SL.
			if (!Windowing.TryComputeBaselineExitUtc (from, nyTz, out var to))
				return DayOutcome.None;
			...
			if (goLong)
				...
				for (int i = start; i < end; i++)
					...
					// Консервативно: если оба “в одной минуте” — считаем SL.
		...
		private static bool TryGetPredDirection ( BacktestRecord r, out bool predUp, out bool predDown )
			...
			if (microUp && microDown)
				throw new InvalidOperationException ($"[model-stats] microUp && microDown одновременно на {r.DateUtc:O}.");
