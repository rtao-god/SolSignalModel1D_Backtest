...
namespace SolSignalModel1D_Backtest.Core.Analytics.Backtest
	...
	public static class SlPnlReportPrinter
			...
			foreach (var w in withSl)
				...
				if (w.Trades != null)
					...
					foreach (var tw in w.Trades)
						...
						bool isGatedDay = gatedDays.Contains (tw.DateUtc.Date)
		...
		private static void ComputeSkipAndExposure (
			...
			var daysWith = new HashSet<DateTime> (withTrades.Select (t => t.DateUtc.Date));
			var daysNo = new HashSet<DateTime> (noTrades.Select (t => t.DateUtc.Date));
