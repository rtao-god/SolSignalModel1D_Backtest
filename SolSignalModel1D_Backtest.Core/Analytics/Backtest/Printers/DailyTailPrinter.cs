...
namespace SolSignalModel1D_Backtest.Core.Analytics.Backtest.Printers
	...
	public static class DailyTailPrinter
		{
		public static void PrintLastDay ( IReadOnlyList<PredictionRecord> records, IEnumerable<BacktestPolicyResult> policyResults )
			{
			...
			head.AddRow ("fact", ClassToStr (rec.TrueLabel) + (rec.FactMicroUp ? " (micro↑)" : rec.FactMicroDown ? " (micro↓)" : ""));
