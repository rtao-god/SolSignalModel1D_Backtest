...
namespace SolSignalModel1D_Backtest.Core.Analytics.Backtest.Printers
	...
	/// - с микро+SL (Total).
	/// Никакой новой математики не считает, только усредняет уже посчитанные поля PredictionRecord.
	/// </summary>
	public static class AggregationProbsPrinter
		...
		/// </summary>
		/// <param name="records">PredictionRecord с уже посчитанными Prob*_Day / Prob*_DayMicro / Prob*_Total и Conf_*.</param>
		/// <param name="trainUntilUtc">Граница train: DateUtc &lt;= trainUntil → Train, &gt; → OOS.</param>
		/// <param name="recentDays">Размер recent-окна в днях (по DateUtc).</param>
		/// <param name="debugLastDays">Сколько последних дней показать в подробной табличке.</param>
		...
		private static void AddSegmentMetaRow ( TextTable t, string name, IReadOnlyList<PredictionRecord> seg )
			{
		...
		// =====================================================================
		// Агрегаты по вероятностям и confidence
		// =====================================================================

		private static void PrintSegmentAverages ( string title, IReadOnlyList<PredictionRecord> seg )
			{
			...
			double sumConfSlLong = 0.0;
			double sumConfSlShort = 0.0;
			...
			foreach (var r in seg)
				...
				sumConfSlLong += r.Conf_SlLong;
				sumConfSlShort += r.Conf_SlShort;

				if (r.Conf_SlLong > 0.0 || r.Conf_SlShort > 0.0 || r.SlProb > 0.0)
					slNonZero++;
				}
			...
			double avgConfSlLong = sumConfSlLong * invN;
			double avgConfSlShort = sumConfSlShort * invN;

			// Sanity-check: суммы вероятностей по слоям должны быть близки к 1, а не к 0.
			// Если хотя бы для одного слоя средняя сумма почти нулевая, значит поля Prob* не были заполнены.
			...
			if (degenerateDay || degenerateDm || degenerateTot)
				...
				Console.WriteLine (
					...
					"Conf_Day={10}, Conf_Micro={11}, Conf_SlLong={12}, Conf_SlShort={13}",
					first.DateUtc,
					first.ProbUp_Day,
					first.ProbFlat_Day,
					first.ProbDown_Day,
			...
			confTable.AddRow ("Conf_SlLong (avg)", FormatProb (avgConfSlLong));
			confTable.AddRow ("Conf_SlShort (avg)", FormatProb (avgConfSlShort));
		...
		// =====================================================================
		// Подробная debug-таблица по последним дням
		// =====================================================================

		private static void PrintLastDaysDebug ( IReadOnlyList<PredictionRecord> last )
			{
