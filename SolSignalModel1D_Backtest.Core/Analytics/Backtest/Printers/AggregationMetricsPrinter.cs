...
namespace SolSignalModel1D_Backtest.Core.Analytics.Backtest.Printers
	...
	///   - Total        (PredLabel_Total, Prob*_Total).
	/// </summary>
	public static class AggregationMetricsPrinter
		...
		private static void PrintSegmentMetrics ( string title, IReadOnlyList<PredictionRecord> seg )
			{
		...
		private static LayerMetrics ComputeLayerMetrics (
			IReadOnlyList<PredictionRecord> seg,
			string layerName,
			Func<PredictionRecord, int> predSelector,
			Func<PredictionRecord, (double up, double flat, double down)> probSelector )
			{
			...
			foreach (var r in seg)
				...
				// Для logloss:
				// - если p_true == 0 → этот день имеет бесконечный loss;
				//   агрегированный logloss тогда не определён, такие дни не включаем,
				//   но считаем их явно и логируем;
				// - если p_true > 0 → учитываем в сумме.
				if (pTrue <= 0.0)
					{
					invalidForLogLoss++;
					}
