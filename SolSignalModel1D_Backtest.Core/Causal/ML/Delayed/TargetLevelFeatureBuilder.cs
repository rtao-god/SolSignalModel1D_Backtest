...
namespace SolSignalModel1D_Backtest.Core.Causal.ML.Delayed
	...
	public static class TargetLevelFeatureBuilder
		...
		public static float[] Build (
			...
			// Жёсткие контракты входов: отсутствие данных/битые цены — это ошибка пайплайна.
			...
			// Важно: строгий каузальный срез по времени.
			var last6 = candles1h
				.Where (c => c.OpenTimeUtc < entryUtc && c.OpenTimeUtc >= from)
				.OrderBy (c => c.OpenTimeUtc)
				.ToList ();

			// Для стабильного набора фич нужен полный 6h-контекст.
			// Если данных меньше — это не "нормально", это дыра в истории/окнах.
			if (last6.Count < 2)
			...
			// Фичи "1h/3h/6h назад" считаем относительно closeNow.
			// Если истории не хватает — надо чинить источники данных.
			...
			// 1h/3h/6h назад относительно closeNow (т.е. -2h/-4h/-6h от entry, если entry на границе часа)
