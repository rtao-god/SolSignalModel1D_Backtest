...
namespace SolSignalModel1D_Backtest.Core.Causal.ML.Delayed.Trainers
	...
	public sealed class TargetLevelPrediction
		...
		public double[] GetProbabilitiesOrThrow ()
			{
			if (Score == null || Score.Length == 0)
				throw new InvalidOperationException ("[target] Score is null or empty.");
		...
		private static double[] Softmax ( float[] scores )
			...
			for (int i = 0; i < scores.Length; i++)
				...
				if (!double.IsFinite (v))
					throw new InvalidOperationException ($"[target] Score contains non-finite value at i={i}: {v}.");
				if (v > max) max = v;
				}
			...
			if (sum <= 0.0 || !double.IsFinite (sum))
				throw new InvalidOperationException ($"[target] Softmax sum invalid: {sum}.");
