...
namespace SolSignalModel1D_Backtest.Core.Causal.ML.Delayed
{
    ...
    public static class TargetLevelFeatureBuilder
    {
            ...
            IReadOnlyList<Candle1h>? candles1h)
        {
            ...
            var tf1h = TimeSpan.FromHours(1);

            var last6 = candles1h
                .Where(c => c.OpenTimeUtc >= from && c.OpenTimeUtc + tf1h <= entryUtc)
                .OrderBy(c => c.OpenTimeUtc)
                .ToList();

            if (last6.Count < 6)
                throw new InvalidOperationException(
                    $"[delayed-feats] insufficient 1h history in [{from:O}, {entryUtc:O}). count={last6.Count}, required>=6.");

            for (int i = 0; i < last6.Count; i++)
            {
                var c = last6[i];
                ...
                {
                    throw new InvalidOperationException(
                        $"[delayed-feats] leakage guard: 1h candle closes after entry. " +
