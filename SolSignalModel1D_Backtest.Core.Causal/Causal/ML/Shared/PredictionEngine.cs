...
using SolSignalModel1D_Backtest.Core.Causal.ML.Aggregation;
...
namespace SolSignalModel1D_Backtest.Core.Causal.ML.Shared
{
    ...
    public sealed class PredictionEngine
    {
        ...
        private PredResult PredictInternal(CausalDataRow row)
        {
            try
            {
                ...
                lock (_sync)
                {
                    ...
                    double pMove = GetProbabilityOrThrow(moveOut.Probability, moveOut.Score, "[PredictionEngine] invalid move probability");
                    ...
                    double pUpGivenMove = GetProbabilityOrThrow(dirOut.Probability, dirOut.Score, "[PredictionEngine] invalid dir probability");
        ...
        private static CausalPredictionRecord ToCausalRecord(CausalDataRow row, PredResult res)
        {
            ...
            {
                TradingEntryUtc = row.TradingEntryUtc,
        ...
        private MicroInfo RunMicroIfAvailable(DateTime dayKeyUtc, float[] fixedFeatures, MLContext ml)
        {
            ...
            double pUp = GetProbabilityOrThrow(microOut.Probability, microOut.Score, "[micro] invalid probability from model");
        ...
        private static double GetProbabilityOrThrow(float prob, float score, string prefix)
        {
            double p = prob;
            ...
            if (!double.IsFinite(p) || p <= 0.0 || p >= 1.0)
            {
                ...
                if (!double.IsFinite(s))
                    throw new InvalidOperationException($"{prefix}: score is not finite ({s}).");

                p = Sigmoid(s);
            }

            ValidateProbabilityOrThrow(p, prefix);
            return p;
        }
        ...
        private static double Sigmoid(double x)
        {
            ...
            double zNeg = Math.Exp(x);
            return zNeg / (1.0 + zNeg);
        }
