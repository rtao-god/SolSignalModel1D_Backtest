	...
	public sealed class MicroDatasetBuilderLeakageTests
		...
		public void Build_UsesOnlyRowsUpToTrainUntil ()
			...
			for (int i = 0; i < 50; i++)
				...
				// Инвариант для этих тестов: IsMorning должен быть валиден (bool),
				// поэтому используем CausalDataRow (а не CausalPredictionRecord.IsMorning, который nullable).
			...
			// trainUntil трактуем как day-key (00:00 UTC), чтобы сравнения были однозначны.
			var trainUntil = startEntryUtc.AddDays (30).ToCausalDateUtc ();
			...
			Assert.All (ds.TrainRows, r => Assert.True (CausalTimeKey.DayKeyUtc (r) <= trainUntil));
			Assert.All (ds.MicroRows, r => Assert.True (CausalTimeKey.DayKeyUtc (r) <= trainUntil));
			}
		...
		public void Build_MicroRowsAreSubsetOfTrainRows ()
			...
			var trainUntil = startEntryUtc.AddDays (40).ToCausalDateUtc ();
		...
		private static CausalDataRow MakeCausalRow (
			...
			// - DateUtc (UTC),
			// - IsMorning (bool),
			// - валидный конечный FeaturesVector фиксированной длины.
			// Остальные значения — детерминированные finite-числа, чтобы не было NaN/Infinity.
			double s = seed;
