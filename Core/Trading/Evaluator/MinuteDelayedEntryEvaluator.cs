...
namespace SolSignalModel1D_Backtest.Core.Backtest
	...
	/// <summary>
	/// Логика отложенного (улучшенного) входа в тот же самый baseline-день.
	/// Окно моделирования = [dayStartUtc; t_exit), t_exit = ComputeBaselineExitUtc(dayStartUtc, nyTz).
	/// </summary>
	public static class DelayedEntryEvaluator
		...
		/// <summary>
		/// Главный метод: попытаться войти лучше, чем в 12:00, в окне [dayStartUtc; t_exit).
		/// </summary>
		public static DelayedEntryResult Evaluate (
			...
			// baseline-окно вместо жёсткого +24h
			DateTime endUtc = Windowing.ComputeBaselineExitUtc (dayStartUtc, nyTz);
			...
			// найдём час, в котором эту цену дали (ограничение по maxDelayHours от dayStartUtc остаётся)
			...
			// ничего не сработало — intraday-результат None до t_exit
			res.Result = DelayedIntradayResult.None;
			return res;
			}
