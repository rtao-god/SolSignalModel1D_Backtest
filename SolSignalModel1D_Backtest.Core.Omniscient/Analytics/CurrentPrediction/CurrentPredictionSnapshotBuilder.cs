...
namespace SolSignalModel1D_Backtest.Core.Omniscient.Analytics.CurrentPrediction
{
    ...
    {
        private static bool _forwardWarningPrinted;
            ...
            int historyWindowDays)
        {
            ...
            if (forwardCount > 0)
            {
                ...
                WarnForwardIfNeeded(forwardCount, firstDayUtc, lastDayUtc);
            }
        ...
        private static bool HasForward(BacktestRecord rec)
        {
            if (rec == null) return false;
            return rec.MaxHigh24 > 0.0 && rec.MinLow24 > 0.0 && rec.Close24 > 0.0;
        }

        private static void WarnForwardIfNeeded(int count, DateTime firstDayUtc, DateTime lastDayUtc)
        {
            if (count <= 0) return;
            if (_forwardWarningPrinted) return;

            _forwardWarningPrinted = true;

            Console.WriteLine(
                $"[current] ВНИМАНИЕ: Forward24h присутствует в CurrentPredictionSnapshot (omniscient). " +
                $"count={count}, range=[{firstDayUtc:yyyy-MM-dd}..{lastDayUtc:yyyy-MM-dd}]. " +
                "Если это используется как «текущий прогноз», это утечка будущего.");
        }
    }
}
