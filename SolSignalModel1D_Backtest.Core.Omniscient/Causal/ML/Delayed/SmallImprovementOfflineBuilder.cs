    ...
    public static class SmallImprovementOfflineBuilder
    {
            ...
            Dictionary<DateTime, Candle6h> sol6hDict)
        {
            ...
            foreach (var r in rows)
            {
                ...
                if (day6.OpenTimeUtc != entryUtc)
                {
                    throw new InvalidOperationException(
                        $"[small-impr-offline] 6h OpenTimeUtc mismatch for entryUtc. entryUtc={entryUtc:O}, day6.OpenTimeUtc={day6.OpenTimeUtc:O}.");
                }

                double entry = day6.Open;
